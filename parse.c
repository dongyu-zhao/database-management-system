...
int all_null(size_t len, char *in[]) {
            ...
            char *filter_h[], char *join_ls_h[][MAX_COLS], char *join_rs_h[][MAX_COLS], char *join_out_h[][MAX_COLS], int join_filter_h[][MAX_COLS])
{
  ...
  // strcat(filename, filter_cols[0]);
  // filter_h[ix ++] = filename;
...
//
// int filter(char *table, int32_t key, int len, char *vales[])
// {
//   FILE *ifp, *ofp;
//   int32_t *key_buffer, *out_buffer, *in_buffers[len];
//   const size_t ROW_SIZE = INT_BUFFER_LENGTH - INT_BUFFER_LENGTH % len;
